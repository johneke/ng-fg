...
angular.module('formDirect', [])
	...
	.directive('formDirectObject', function($compile, getobject){
		...
		var linker = function(scope, element, attrs) {
			...
			switch (schema.type.toLowerCase())
			{
				case 'object':
					...
					for (i in schema.properties)
					{
						...
						switch (field.type.toLowerCase())
						{
							...
							case 'number':
							case 'string':
								content +=	'<form-direct-control form-direct-control-type="{type}" form-direct-object-name="{name}"></form-direct-control><br />'
				...
				case 'number':
				case 'string':
					template = '<form-direct-control form-direct-control-type="{type}"></form-direct-control><br />'.replace(/{type}/gi, schema.type.toLowerCase());
					break;
	...
	.directive('formDirectArrayItem', function($compile, getobject){
		...
		var linker = function(scope, element, attrs) {
			...
			switch(schema.type.toLowerCase())
			{
				...
				case 'number':
				case 'string':
					content = '<form-direct-control form-direct-control-type="{type}"></form-direct-control><br />'.replace(/{type}/gi, schema.type.toLowerCase());
					break;
	...
	.directive('formDirectControl', function($compile, getobject){

		var linker = function(scope, element, attrs) {
			var template = null;
			switch (attrs.formDirectControlType)
			{
				case 'boolean':
					template = '<input type="checkbox" placeholder="{name}" />';
					break;
			...
			template = template.replace(/{name}/gi, attrs.formDirectObjectName);
