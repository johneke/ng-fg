...
angular.module('formDirect', [])
	.directive('formDirectGenerate', function($compile){
		var linker = function(scope, element, attrs) {
			...
			function getTemplate(field, model)
			{
				...
				switch(Object.prototype.toString.call(field))
				{
					case '[object Object]':
						...
						for (i in field)
						{
							switch(Object.prototype.toString.call(field[i]))
							{
								...
								case '[object Array]':
									content += '<fieldset {group-class}>';
									content += '<legend>{legend}<button {add-btn-class} data-ng-click="{id}.push({obj})">Add</button></legend>'
					...
					case '[object Array]':
						...
						content += '<button {remove-btn-class} data-ng-click="{model}.splice($index, 1)" data-ng-disabled="{model}.length < 2">Remove</button>'.replace(/{model}/gi, model);
						content += '</div>';
						break;
			...
			var template = '<form {form-class}>' + getTemplate(scope.fdrData, scope.fdrModelName) + '</form>';
